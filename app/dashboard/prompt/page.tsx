...
const formSchema = z.object({
  ...
  greetingPrompt: z.string().min(1, { message: "Welcome message cannot be empty." }),
})
...
useEffect(() => {
  async function fetchPrompts() {
    try {
      ...
      const greetRes = await fetch("https://ai.rajatkhandelwal.com/greeting", {
        headers: getAuthHeaders(),
      });
      if (!greetRes.ok) throw new Error("Failed to fetch greeting prompt");
      const greetData = await greetRes.json();
      // Fallback: supports both {greetingPrompt: "..."} or {greeting: "..."}
      form.setValue("greetingPrompt", greetData.greetingPrompt || greetData.greeting || "");
    } catch (error) {
...
async function onSubmit(values: z.infer<typeof formSchema>) {
  ...
  try {
    ...
    const greetRes = await fetch("https://ai.rajatkhandelwal.com/setgreeting", {
      method: "POST",
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      This message will be shown as the AI's initial greeting to users.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
